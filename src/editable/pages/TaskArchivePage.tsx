      ...
      <main className="min-h-screen bg-[#e7f1f5] text-[#26394f]">
        <section className="bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#4daf73]">Media distribution</p>
              <h1 className="mt-3 text-5xl font-black leading-none text-[#2c5e9d] sm:text-6xl lg:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md border-l-4 border-[#4daf73] pl-5 text-sm font-bold leading-7 text-[#63727e]">
              Timely releases, media-ready summaries, and category-led discovery organized for fast browsing.
            </p>
          </div>
        </section>
        ...
        <section className="bg-[#2c5e9d] text-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[#b8e6c7]' : 'hover:text-[#b8e6c7]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#b8e6c7]' : 'whitespace-nowrap hover:text-[#b8e6c7]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] bg-white shadow-[0_16px_44px_rgba(44,94,157,.12)] lg:grid-cols-[1.75fr_0.75fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#123e70] via-[#123e70]/15 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="bg-[#4daf73] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em]">{getCategory(lead, label)}</span>
            ...
            <div className="grid">
              <div className="bg-[#2c5e9d] p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight">What the network is watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-[#d8e6ec] bg-white last:border-b-0">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover grayscale transition group-hover:grayscale-0" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#4daf73]">0{index + 1}</p>
                    <h3 className="mt-3 text-xl font-black leading-tight text-[#0052a3]">{post.title}</h3>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] bg-[#e7f1f5] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 pb-4">
            <h2 className="text-4xl font-black text-[#4daf73] sm:text-5xl">Latest News</h2>
            <form action={basePath} className="flex border border-[#b8cbd5] bg-white">
              ...
              <button className="h-11 bg-[#2c5e9d] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group bg-white shadow-[0_10px_24px_rgba(44,94,157,.1)]">
                  <div className="aspect-[16/10] overflow-hidden bg-[#d9e8ed]">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.18em] text-[#4daf73]">
                      <span>{getCategory(post, label)}</span><span>{String(index + 3).padStart(2, '0')}</span>
                    </div>
                    <h3 className="mt-4 text-2xl font-black leading-tight text-[#0052a3]">{post.title}</h3>
                    <p className="mt-4 line-clamp-3 text-sm leading-6 text-[#63727e]">{getSummary(post)}</p>
                  </div>
                </Link>
              ))}
            </div>
          ) : !lead ? (
            <div className="border border-dashed border-[#b8cbd5] bg-white p-12 text-center">
              <Search className="mx-auto h-8 w-8" />
              <h2 className="mt-4 text-3xl font-black text-[#2c5e9d]">No stories found</h2>
              <p className="mt-2 text-sm text-[#63727e]">Try another category or publish a new update.</p>
            </div>
          ) : null}

          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="border border-[#b8cbd5] bg-white px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
            <span className="bg-[#4daf73] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="border border-[#b8cbd5] bg-white px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
