...
function ArticleDetail({ task, post, related, comments }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="bg-[#e7f1f5] text-[#26394f]">
      <header className="bg-white shadow-[0_10px_28px_rgba(44,94,157,.1)]">
        <div className="mx-auto max-w-[1180px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t-4 border-[#4daf73] pt-4 text-[11px] font-black uppercase tracking-[0.16em]">
            <span className="text-[#4daf73]">{categoryOf(post, 'News')}</span>
            {published ? <time>{published}</time> : null}
          </div>
          <h1 className="mt-6 max-w-6xl text-4xl font-black leading-tight text-[#0052a3] sm:text-5xl lg:text-6xl">{post.title}</h1>
          {summaryText(post) ? <p className="mt-6 max-w-4xl text-xl font-bold leading-8 text-[#63727e] sm:text-2xl">{summaryText(post)}</p> : null}
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto max-w-[1180px] bg-white shadow-[0_12px_30px_rgba(44,94,157,.12)]">
          <img src={images[0]} alt="" className="max-h-[760px] w-full object-cover" />
          <figcaption className="border-t border-[#d7e4eb] px-4 py-3 text-xs italic text-[#63727e] sm:px-6">Featured image for {post.title}</figcaption>
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_300px] lg:px-8 lg:py-16">
        <article className="min-w-0 bg-white p-6 shadow-[0_12px_30px_rgba(44,94,157,.1)] sm:p-8">
          <BodyContent post={post} />
          <EditableComments slug={post.slug} comments={comments} />
        </article>
        <div className="border-t-4 border-[#4daf73] pt-5">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-[#d7e4eb] py-3 transition hover:text-[#4daf73]">
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 border-t-4 border-[#4daf73] bg-[#f7fbfd] p-5">
