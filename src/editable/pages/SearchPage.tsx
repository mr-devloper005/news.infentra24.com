...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden bg-white shadow-[0_10px_24px_rgba(44,94,157,.1)] ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-black ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="absolute left-4 top-4 bg-[#4daf73] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="bg-[#4daf73] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span> : null}
        <h2 className="mt-4 line-clamp-3 text-2xl font-black leading-tight text-[#0052a3]">{post.title}</h2>
        {summary ? <p className="mt-4 line-clamp-3 text-sm font-semibold leading-7 text-[#63727e]">{summary}</p> : null}
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#e7f1f5] text-[#26394f]">
        <section className="mx-auto max-w-[1180px] bg-[#e7f1f5]">
          <div className="grid bg-white shadow-[0_12px_32px_rgba(44,94,157,.12)] md:grid-cols-[0.8fr_1.2fr]">
            <div>
              <div className="h-full bg-[#2c5e9d] p-7 text-white sm:p-10 lg:p-14">
                <p className="text-xs font-black uppercase tracking-[0.28em]">{pagesContent.search.hero.badge}</p>
                <h1 className="mt-5 text-4xl font-black leading-tight sm:text-6xl">{pagesContent.search.hero.title}</h1>
            ...
            <form action="/search" className="self-center p-6 sm:p-10 lg:p-14">
              ...
              <label className="flex items-center gap-3 border border-black bg-white px-4 py-3">
                <Search className="h-5 w-5 text-[#4daf73]" />
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center bg-[#2c5e9d] px-6 text-xs font-black uppercase tracking-[0.18em] text-white transition hover:bg-[#4daf73]" type="submit">Search</button>
            </form>
          </div>

          <div className="flex flex-wrap items-end justify-between gap-4 px-4 py-8 sm:px-6 lg:px-8">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.24em] text-[#4daf73]">{results.length} results</p>
              <h2 className="mt-2 text-4xl font-black text-[#2c5e9d]">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
            <Link href="/article" className="nv-blue-button">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
